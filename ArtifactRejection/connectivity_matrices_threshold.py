...
folder_input = pathlib.Path('S:/OneDrive/PythonProjects/Connectivity/Network Analysis/Input_old/')#pathlib.Path('S:/OneDrive/PythonProjects/ArtifactsRejection/output_v3/')
folder_output =  folder_output = pathlib.Path('S:/OneDrive/PythonProjects/Connectivity/Network Analysis/R_Input/')#main_folder/'con_matrices' 

files = [x for x in folder_input.iterdir() if x.is_file() and x.suffix == '.edf' ]
...
bem = mne.read_bem_solution('Network Analysis/Auxiliary/bem_oct6')
src = mne.read_source_spaces('Network Analysis/Auxiliary/src_oct6')

montage = mne.channels.read_custom_montage("Network Analysis/Auxiliary/64-cap_new.bvef")

#‘aparc’ or ‘aparc.a2009s’. 'HCPMMP1',
labels = mne.read_labels_from_annot('fsaverage', 'aparc') #aparc - 68 labels, aparc 2009 - 250
...
def graph_measures_from_con_matrices(matrices, thresholds, node_names, con_freqs, file, folder_output):
...
# =============================================================================
    try:    
        ...
        person = file.stem.split('_')[0]
        path = f'{fol}/{person}_Markers'
        ...
        con, freqs, times, n_epochs, n_tapers = mne.connectivity.spectral_connectivity(labels_ts, 
                                                                      method='imcoh', mode='multitaper', sfreq=epochs.info['sfreq'], 
                                                                      fmin=freqs_min, fmax=freqs_max, faverage=True, 
                                                                      mt_adaptive=True, n_jobs=-1)
        
        for shp in range(0, con.shape[2]):
            output_filename = f'{folder_output}/{person}_ZG_6-sec_sources_imcoh_{con_freqs[shp]}.csv'
            np.savetxt(output_filename, np.array(con[:, :, shp]), delimiter=',')
...
file = [x for x in files if '1112' in x.stem]
